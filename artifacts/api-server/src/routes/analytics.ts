...
import { requireAuth, requireDirector, isFallbackMode } from '../lib/auth';
import { fallbackBuffet, fallbackThawing, fallbackReceived, fallbackDisinfection } from '../lib/fallback-log-store';
import { fallbackManagers } from '../lib/fallback-manager-store';
...
router.get('/', async (_req, res, next) => {
  try {
    let bAll: any[], tAll: any[], rAll: any[], dAll: any[], managerList: any[];
    if (isFallbackMode()) {
      bAll = fallbackBuffet.all();
      tAll = fallbackThawing.all();
      rAll = fallbackReceived.all();
      dAll = fallbackDisinfection.all();
      managerList = fallbackManagers.list();
    } else {
      const week = daysAgo(7);
      [bAll, tAll, rAll, dAll, managerList] = await Promise.all([
    ...
    res.json(buildAnalytics(bAll, tAll, rAll, dAll, managerList));
  } catch (err) { next(err); }
});
...
router.get('/:hotel', async (req, res, next) => {
  try {
    ...
    if (isFallbackMode()) {
      res.json({
        hotel,
        buffet: fallbackBuffet.list({ hotel }),
        thawing: fallbackThawing.list({ hotel }),
        received: fallbackReceived.list({ hotel }),
        disinfection: fallbackDisinfection.list({ hotel }),
      });
      return;
    }
